...
def _normalize_candidates(url: str):
    candidates = [url]
...
def _fetch_with_scraperapi(url: str, timeout: int = 30) -> requests.Response:
    key = os.getenv("SCRAPERAPI_KEY", "").strip()
    if not key:
        raise RuntimeError("Site trả 403. Hãy dùng HTTPS hoặc cấu hình SCRAPERAPI_KEY để proxy bypass anti-bot.")
    endpoint = "http://api.scraperapi.com"
    params = {"api_key": key, "url": url, "render": "false", "country_code": "us"}
...
def get_soup(url: str) -> BeautifulSoup:
    last_error = None
    for candidate in _normalize_candidates(url):
        try:
            r = client().get(candidate, timeout=30, allow_redirects=True)
            if r.status_code == 403:
                r = _fetch_with_scraperapi(candidate, timeout=30)
            r.raise_for_status()
            ...
            last_error = e
            continue
    raise RuntimeError(f"Không tải được trang: {last_error}")
